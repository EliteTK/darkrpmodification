...
DarkRP.notify(activator, 0, 3, "You purchased a MP7 for " .. DarkRP.formatMoney(cash) .. "!")
...
if gain == 0 then
DarkRP.notify(owner, 3, 3, DarkRP.getPhrase("you_received_x", DarkRP.formatMoney(0) .. " " .. DarkRP.getPhrase("profit"), "MP7 (" .. DarkRP.getPhrase("gun_lab") .. ")"))
else
owner:addMoney(gain)
local word = DarkRP.getPhrase("profit")
if gain < 0 then word = DarkRP.getPhrase("loss") end
DarkRP.notify(owner, 0, 3, DarkRP.getPhrase("you_received_x", DarkRP.formatMoney(math.abs(gain)) .. " " .. word, "MP7 (" .. DarkRP.getPhrase("gun_lab") .. ")"))
end
end
...
gun:SetModel("models/weapons/w_mp7_silenced.mdl")
gun:SetWeaponClass("m9k_mp7")
